...
class ModelConfig:
    def __init__(self):
        ...
        logger_init(log_file_name='pair', log_level=logging.INFO, log_dir=self.logs_save_dir)
...
def train(config):
    ...
    train_iter, test_iter, val_iter = \
        data_loader.load_train_val_test_data(config.train_file_path, config.val_file_path, config.test_file_path)
    lr_scheduler = get_scheduler(name='linear', optimizer=optimizer, num_warmup_steps=int(len(train_iter) * 0),
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (sample, seg, label) in enumerate(train_iter):
            ...
            loss, logits = model(input_ids=sample, attention_mask=padding_mask,
                                 token_type_ids=seg, position_ids=None, labels=label)
            optimizer.zero_grad()
            loss.backward()
            lr_scheduler.step()
            optimizer.step()
