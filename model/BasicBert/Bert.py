...
class BertModel(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None):
        """
        ***** 一定要注意，attention_mask中，被mask的Token用1(True)表示，没有mask的用0(false)表示：这一点一定一定要注意
    ...
    def from_pretrained(cls, config, pretrained_model_dir=None):
        model = cls(config)  # 初始化模型，cls为未实例化的对象，即一个未实|例化的BertModel对象

        # todo train
        # pretrained_model_path = os.path.join(pretrained_model_dir, "pytorch_model.bin")
        # todo inference
        pretrained_model_path = os.path.join(pretrained_model_dir, "model_giikin.bin")
        ...
        loaded_paras = torch.load(
            pretrained_model_path,
            # 不指定 GPU RuntimeError: storage has wrong size: expected 5817902493874292617 got 2359296
