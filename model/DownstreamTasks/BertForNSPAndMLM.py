...
class BertForPretrainingModel(nn.Module):
    ...
    def __init__(self, config, bert_pretrained_model_dir=None):
        ...
        tmp = config.__dict__
        if bert_pretrained_model_dir is not None:  # 模型加载
            self.bert = BertModel.from_pretrained(config, bert_pretrained_model_dir)
    ...
    def forward(
            self, input_ids,  # [src_len, batch_size]
            attention_mask=None,  # [batch_size, src_len] mask掉padding部分的内容
            token_type_ids=None,  # [src_len, batch_size]
            position_ids=None,
            masked_lm_labels=None,  # [src_len,batch_size]
            next_sentence_labels=None):  # [batch_size]
        pooled_output, all_encoder_outputs = self.bert(input_ids=input_ids,
                                                       attention_mask=attention_mask,
                                                       token_type_ids=token_type_ids,
                                                       position_ids=position_ids)
        sequence_output = all_encoder_outputs[-1]  # 取Bert最后一层的输出
        ...
        if masked_lm_labels is not None and next_sentence_labels is not None:  # Train Data需要计算损失
            ...
            mlm_loss = loss_fct_mlm(
                mlm_prediction_logits.reshape(-1, self.config.vocab_size), masked_lm_labels.reshape(-1)
            )
            nsp_loss = loss_fct_nsp(nsp_pred_logits.reshape(-1, 2), next_sentence_labels.reshape(-1))
            total_loss = mlm_loss + nsp_loss
            return total_loss, mlm_prediction_logits, nsp_pred_logits
        else:  # Test Data 就不需要计算损失了
