...
from utils.data_helpers import pad_sequence
import torch
...
"""
        line1 = i[:int(len(i) / 2)]
        line2 = i[int(len(i) / 2):]
        newAllData[idx].append(line1)
        newAllData[idx].append(line2)
    ...
    for line in tqdm(lines, ncols=80, desc=" ## 正在读取原始数据"):
        ...
        for w in line[1:]:
            if paragraphs[-1][-1][-1] in seps:
                ...
                paragraphs[-1][-1] += w
    random.shuffle(paragraphs)  # 将所有段落打乱
    return paragraphs  # 句子的【分句】处理


def read_giikin_fzm0810_task_(filepath=None, seps='，'):
    ...
    appendTmp = []
    for continueSplit in tmp:
        for ic in continueSplit:
            if random.random() < 0.7 and len(ic) >= 4:  # 产生[0,1)之间的一个随机数  切分
                appendTmp.append([])
                line1 = ic[:int(len(ic) / 2)]
                line2 = ic[int(len(ic) / 2):]
                appendTmp[len(appendTmp) - 1].append(line1)
                appendTmp[len(appendTmp) - 1].append(line2)
            else:  # 不进行切分
                pass
...
def read_GiikinFake(filepath=None):
    outbd = ['。', '；', '！', '？']
    step1seqs = []  # 。
    step2seqs = []  # ；
    step3seqs = []  # ！
    step4seqs = []  # ？
    with open('../data/GiikinAds/train.json', 'r', encoding='utf-8') as fTrain:
        trainData = fTrain.readlines()
        for line in trainData:
            lineData = json.loads(line)
            summary = lineData['summary']
            for eobd in outbd:  # 。
                sequences = summary.split(eobd)
                for eachSeq in sequences:
                    if eachSeq:
                        step1seqs.append(eachSeq)
                break

        for eachSeqFroms1 in step1seqs:
            for min in eachSeqFroms1.split('；'):
                step2seqs.append(min)

        for eachSeqFroms2 in step2seqs:
            for min in eachSeqFroms2.split('！'):
                step3seqs.append(min)

        for eachSeqFroms3 in step3seqs:
            if "<UNK>" in eachSeqFroms3:
                continue
            if not eachSeqFroms3:
                continue
            for min in eachSeqFroms3.split('？'):
                step4seqs.append(min)
                # fTrainW.write(min + '\n')
...
def read_giikinFakeDev():
    outbd = ['。', '；', '！', '？']
    step1seqs = []  # 。
    step2seqs = []  # ；
    step3seqs = []  # ！
    step4seqs = []  # ？
    with open('../data/GiikinAds/dev.json', 'r', encoding='utf-8') as f:
        data = f.readlines()
        for line in data:
            lineData = json.loads(line)
            summary = lineData['summary']
...
def read_giikinTrue(filepath=None):
    giikinTrueData = []
    giikinTrueSplitData = []
    with open('../data/GiikinAds/merge_sentence.txt', 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            line = line.replace('\'', '"')
            line = json.loads(line)
            if '1' in list(line.keys()):
                giikinTrueData.append(''.join(line["1"]))
                giikinTrueSplitData.append(line['1'])
    return giikinTrueData, giikinTrueSplitData
    # raise NotImplementedError("本函数为实现，请参照`read_songci()`或`read_wiki2()`返回格式进行实现")


def read_giikinTureNOTMerge():
    count = 0
    allDataNOTMerge = []
    try:
        with open('../data/GiikinAds/not_merge_sentence.txt', 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for idx, i in enumerate(lines):
                data = i.replace('\'', '"')
                data = json.loads(data.strip())
                data = data['text']
                data = re.sub('[·★…【】[\\]^_`{|}~\s]+', "", data)
                data = re.sub(
                    '[\001\002\003\004\005\006\007\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a]+',
                    '', data)
        allDataNOTMerge.append(data)
    except:
        count += 1
    # print(count)
    return allDataNOTMerge
...
"""
# # todo 预处理持久化Giikin数据
giikinFakeData = read_GiikinFake('')  # fake data train
giikinFakeDataDev = read_giikinFakeDev()  # fake data dev
giikinTrueData, giikinTrueSplitData = read_giikinTrue('')  # true data need merge
giikinTrueData = list(set(giikinTrueData))
# 数据 clean     giikinTrueData
newGiikinTrueData = []
for s in giikinTrueData:
    s = re.sub('[·★…【】[\\]^_`{|}~\s]+', "", s)
    s = re.sub(
        '[\001\002\003\004\005\006\007\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a]+',
        '', s)
    s = s.replace('\\', '')
    newGiikinTrueData.append(s)
giikinTrueDataNeedMerge = newGiikinTrueData
del newGiikinTrueData
gc.collect()
giikinTrueDataNOTMerge = read_giikinTureNOTMerge()  # true data not merge
allGiikinFakeData = giikinFakeData + giikinFakeDataDev
allGiikinTrueData = giikinTrueDataNeedMerge + giikinTrueDataNOTMerge

allData = allGiikinTrueData + allGiikinFakeData
newAllData = []
for idx, i in enumerate(allData):
    newAllData.append([])
    if not i:
        continue
    if '，' in i:
        i_split = i.split('，')
        newAllData[idx] += i_split
    else:
        line1 = i[:int(len(i) / 2)]
        line2 = i[int(len(i) / 2):]
        newAllData[idx].append(line1)
...
train_set_mid, test_set = train_test_split(newAllData, test_size=0.004, random_state=42)
train_set, dev_set = train_test_split(train_set_mid, test_size=0.004, random_state=42)
# print(len(train_set))
# print(len(test_set))
# print(len(dev_set))
...
with open('../data/GiikinAds/giikin.train_fzm0810_task_.pickle', 'wb') as fTrainW:
    pickle.dump(train_set, fTrainW)
with open('../data/GiikinAds/giikin.test_fzm0810_task_.pickle', 'wb') as fTestW:
    pickle.dump(test_set, fTestW)
with open('../data/GiikinAds/giikin.valid_fzm0810_task_.pickle', 'wb') as fDevW:
    pickle.dump(dev_set, fDevW)
...
class LoadBertPretrainingDataset(object):
    ...
    def get_format_data(self, filepath):
        ...
        elif self.data_name == 'giikin':
            # 在这里，可以调用你自己数据对应的格式化函数，但是返回格式需要同read_wiki2()保持一致。
            return read_giikin_fzm0810_task_(filepath, self.seps)
    ...
    def get_masked_sample(self, token_ids):
        ...
        for i, ids in enumerate(token_ids):
            # 在遮蔽语言模型任务中不会预测特殊词元，所以如果该位置是特殊词元那么该位置就不会成为候选mask位置
    ...
    def data_process(self, filepath, postfix='cache'):
        ...
        :return:
        """
        paragraphs = self.get_format_data(filepath)  # todo 这里返回的是为了训练而对数据进行切分过后的数据
        # 返回的是一个二维列表，每个列表可以看做是一个段落（其中每个元素为一句话）
        data = []
        max_len = 0
        # todo IMPORTANT 开始构造数据 !!!!! 这里的max_len用来记录整个数据集中最长序列的长度，在后续可将其作为padding长度的标准
        ...
        for paragraph in tqdm(paragraphs, ncols=80, desc=desc):  # 遍历每个
            for i in range(len(paragraph) - 1):  # 遍历一个段落中的每一句话
                ...
                segs = seg1 + seg2  # CLS + tokens-1 + SEP + tokens-2 + SEP
                ...
                logging.debug(f" ##      segment ids:{segs.tolist()},序列长度为 {len(segs)}")  # tokens_id = segs
    ...
    def data_process_online(self, paragraphs, postfix='cache'):
        data = []  # 返回的是一个二维列表，每个列表可以看做是一个段落（其中每个元素为一句话）
        max_len = 0
        # 这里的max_len用来记录整个数据集中最长序列的长度，在后续可将其作为padding长度的标准
        desc = f" ## 正在构造NSP和MLM样本)"
        for paragraph in tqdm(paragraphs, ncols=80, desc=desc):  # 遍历每个
            for i in range(len(paragraph) - 1):  # 遍历一个段落中的每一句话
                sentence, next_sentence, is_next = self.get_next_sentence_sample(
    ...
    def generate_batch(self, data_batch):
        ...
        b_token_ids = pad_sequence(b_token_ids,  # [batch_size,max_len]
                                   padding_value=self.PAD_IDX, batch_first=False, max_len=self.max_sen_len)
    ...
    def load_train_val_test_data(self, train_file_path=None, val_file_path=None, test_file_path=None, only_test=False):
        # todo name task ID with date / data / or other key information
        " todo important: 这里的postfix是当前处理任务的唯一标识；关系到数据的读取与该任务对应的模型的存储；"
        ...
        postfix = '_fzm0810_task_'
        # 这个def完成的工作是先切分数据，再执行数据集构造，可以看到其下就是进一步的 data Loader obj
        ...
        logging.info(f"## 成功返回训练集样本（{len(train_iter.dataset)}）个、验证集样本（{len(val_iter.dataset)}）个"
                     f"测试集样本（{len(test_iter.dataset)}）个.")
        return train_iter, test_iter, val_iter

    def load_train_val_test_data_online(self, train_file_path=None, val_file_path=None, test_file_path=None,
                                        only_test=False):
        # todo name task ID with date / data / or other key information
        " todo important: 这里的postfix是当前处理任务的唯一标识；关系到数据的读取与该任务对应的模型的存储；"
        # postfix = f"_ml{self.max_sen_len}_rs{self.random_state}_mr{str(self.masked_rate)[2:]}" \
        #           f"_mtr{str(self.masked_token_rate)[2:]}_mtur{str(self.masked_token_unchanged_rate)[2:]}"
        postfix = '_fzm0810_task_'
        # todo 线上推理时简单粗暴直接传入一个语句list即可
        test_data = self.data_process_online(test_file_path, postfix='test' + postfix)['data']
        test_iter = DataLoader(test_data, batch_size=self.batch_size, shuffle=False, collate_fn=self.generate_batch)
        if only_test:
            logging.info(f"## 成功返回测试集，一共包含样本{len(test_iter.dataset)}个")
            return test_iter
    ...
    def make_inference_samples(self, sentences=None, masked=False, language='en', random_state=None):
        ...
        input_tokens_ids = pad_sequence(input_tokens_ids, padding_value=self.PAD_IDX,
                                        batch_first=False, max_len=None)  # 按一个batch中最长的样本进行padding
